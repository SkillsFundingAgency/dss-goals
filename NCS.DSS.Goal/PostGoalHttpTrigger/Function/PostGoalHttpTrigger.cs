...
namespace NCS.DSS.Goal.PostGoalHttpTrigger.Function
{
    public class PostGoalHttpTrigger
    {
        ...
        {
            _logger.LogTrace("Function {FunctionName} has been invoked", nameof(PostGoalHttpTrigger));
            ...
            if (goalRequest == null)
            {
                _logger.LogInformation("{goalRequest} object is NULL. Correlation GUID: {CorrelationGuid}", nameof(goalRequest), correlationGuid);
                return new UnprocessableEntityObjectResult(req);
            }

            _logger.LogTrace("Attempting to set IDs for Goal. Correlation GUID: {CorrelationGuid}", correlationGuid);
            goalRequest.SetIds(customerGuid, actionPlanGuid, touchpointId, subcontractorId);
            _logger.LogTrace("IDs successfully set for Goal. Correlation GUID: {CorrelationGuid}", correlationGuid);

            _logger.LogTrace("Attempting to validate {goalRequest} object", nameof(goalRequest));
            ...
            _logger.LogTrace("Successfully validated {goalRequest} object", nameof(goalRequest));
            ...
            _logger.LogTrace("Attempting to create Goal for customer in Cosmos DB. Customer GUID: {CustomerId}", customerGuid);
            var goal = await _goalsPostService.CreateAsync(goalRequest);

            if (goal == null)
            {
                _logger.LogInformation("Failed to create Goal for customer. Customer GUID: {CustomerId}", customerGuid);
                return new BadRequestObjectResult(customerGuid);
            }

            _logger.LogTrace("Goal created successfully in Cosmos DB. Goal GUID: {GoalId}", goal.GoalId);

            _logger.LogTrace("Attempting to send message to Service Bus Namespace. Goal GUID: {GoalId}", goal.GoalId);
            await _goalsPostService.SendToServiceBusQueueAsync(goal, apimUrl);
            _logger.LogTrace("Successfully sent message to Service Bus. Goal GUID: {GoalId}", goal.GoalId);

            _logger.LogTrace("Function {FunctionName} has finished invoking", nameof(PostGoalHttpTrigger));
