...
namespace NCS.DSS.Goal.PatchGoalHttpTrigger.Function
{
    public class PatchGoalHttpTrigger
    {
        ...
        {
            _logger.LogTrace("Function {FunctionName} has been invoked", nameof(PatchGoalHttpTrigger));
            ...
            {
                _logger.LogInformation("Unable to parse 'goalId' to a GUID. GoalId ID: {GoalId}", goalId);
            ...
            {
                _logger.LogInformation("{goalPatchRequest} object is NULL. Correlation GUID: {CorrelationGuid}", nameof(goalPatchRequest), correlationGuid);
                return new UnprocessableEntityObjectResult(req);
            }

            _logger.LogTrace("Attempting to set IDs for Goal PATCH. Correlation GUID: {CorrelationGuid}", correlationGuid);
            goalPatchRequest.SetIds(touchpointId, subcontractorId);
            _logger.LogTrace("IDs successfully set for Goal PATCH. Correlation GUID: {CorrelationGuid}", correlationGuid);

            _logger.LogTrace("Attempting to validate {goalPatchRequest} object", nameof(goalPatchRequest));
            ...
            _logger.LogTrace("Successfully validated {goalPatchRequest} object", nameof(goalPatchRequest));
            ...
            _logger.LogTrace("Checking if Interaction exists for Customer. Customer GUID: {CustomerId}. Interaction GUID: {InteractionGuid}. Correlation GUID: {CorrelationGuid}", customerGuid, interactionGuid, correlationGuid);
            ...
            _logger.LogTrace("Checking if action plan exists for customer. Customer GUID: {CustomerId}. Action Plan GUID: {ActionPlanGuid}. Correlation GUID: {CorrelationGuid}", customerGuid, actionPlanGuid, correlationGuid);
            ...
            _logger.LogTrace("Attempting to get goal for Customer. Customer GUID: {CustomerId}. Action Plan GUID: {ActionPlanGuid}. Correlation GUID: {CorrelationGuid}", customerGuid, actionPlanGuid, correlationGuid);
            ...
            {
                _logger.LogInformation("Goal not found. Customer GUID: {CustomerId}. Action Plan GUID: {ActionPlanGuid}. Correlation GUID: {CorrelationGuid}", customerGuid, actionPlanGuid, correlationGuid);
                return new NoContentResult();
            }
            _logger.LogTrace("Goal successfully retrieved. Goal GUID: {GoalId}", goalGuid);

            _logger.LogTrace("Attempting to PATCH Goal resource.");
            var patchedGoal = _goalsPatchService.PatchResource(goalForCustomer, goalPatchRequest);

            if (patchedGoal == null)
            {
                _logger.LogInformation("Failed to PATCH Goal resource.");
                return new NoContentResult();
            }

            _logger.LogTrace("Attempting to update Goal in Cosmos DB. Goal GUID: {GoalId}", goalGuid);
            var updatedGoal = await _goalsPatchService.UpdateCosmosAsync(patchedGoal, goalGuid);

            if (updatedGoal == null)
            {
                _logger.LogInformation("Goal update request unsuccessful. Goal GUID: {GoalId}", goalGuid);
                return new BadRequestObjectResult(goalGuid);
            }

            _logger.LogTrace("Goal updated successfully in Cosmos DB. Goal GUID: {GoalId}", goalGuid);

            _logger.LogTrace("Attempting to send message to Service Bus Namespace. Goal GUID: {GoalId}", goalId);
            await _goalsPatchService.SendToServiceBusQueueAsync(updatedGoal, customerGuid, apimUrl);
            _logger.LogTrace("Successfully sent message to Service Bus. Goal GUID: {GoalId}", goalId);

            _logger.LogTrace("Function {FunctionName} has finished invoking", nameof(PatchGoalHttpTrigger));
